<?php
	...
	$appointment = [];
		...
		$sql = 'SELECT apt.user_id, apt.reason, apt.date_time, s.student_lname, s.student_fname, s.student_email, s.student_num, s.student_companion, g.guest_companion, g.guest_fname, g.guest_lname, g.guest_email, g.guest_number, apt.department, apt.faculty, apt.status, apt.id as appointment_id' .
			...
			<td>
				...
				<div class="dialog" data-apt-id="<?php echo $apt['appointment_id'] ?>" title="Appointment Details">
					<div>
						<table>
							<tr>
								<th class="data"><div>Name:</div> <div><div><?php echo $apt[$transaction_type . '_fname']?> <?php echo $apt[$transaction_type . '_lname']?></div>
								</th>
								<th class="data"><div>Reason:</div> <div><?php echo ucwords($apt['reason'])?></div>
								</th>
							</tr>
							<tr>
								<th class="data"><div><?php echo ($transaction_type === "guest" ? "Contact" : "Student")?> Number:</div> <div><?php echo ($transaction_type === "guest" ? $apt['guest_number'] : $apt['student_num'])?></div>
								</th>
								<th class="data"><div>Department:</div> <div><?php echo ucwords($apt['department'])?></div>
								</th>
							</tr>
							<tr>
								<th class="data"><div>Email:</div> <div><?php echo $apt[$transaction_type . '_email']?></div>
								</th>
								<th class="data"><div>Companion:</div> <div><?php echo $apt[$transaction_type . '_companion']?> </div>
								</th>
							</tr>
						</table>
					</div>
